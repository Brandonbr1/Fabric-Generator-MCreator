...
public class ${name}PortalBlock extends NetherPortalBlock {
	...
	@Override public BlockState updateShape(BlockState blockState, Direction direction, BlockState blockState2, LevelAccessor levelAccessor, BlockPos blockPos, BlockPos blockPos2) {
            Axis axis = direction.getAxis();
            Axis axis2 = (Axis)blockState.getValue(AXIS);
            boolean bl = axis2 != axis && axis.isHorizontal();
            return !bl && !blockState2.is(this) && !(new ${name}PortalShape(levelAccessor, blockPos, axis2)).isComplete() ? Blocks.AIR.defaultBlockState() : super.updateShape(blockState, direction, blockState2, levelAccessor, blockPos, blockPos2);
        }
